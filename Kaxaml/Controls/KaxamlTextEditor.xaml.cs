...
public partial class KaxamlTextEditor : IKaxamlInfoTextEditor
{
    ...
    {
        if (obj is KaxamlTextEditor owner) owner.TextEditor.ActiveTextAreaControl.Caret.Line = (int)args.NewValue;
    }
    ...
    {
        if (obj is KaxamlTextEditor owner) owner.TextEditor.ActiveTextAreaControl.Caret.Position = new TextLocation((int)args.NewValue, owner.TextEditor.ActiveTextAreaControl.Caret.Position.Y);
    }
    ...
    {
        if (obj is KaxamlTextEditor owner) owner.TextEditor.ShowLineNumbers = (bool)args.NewValue;
    }
    ...
    {
        if (obj is KaxamlTextEditor owner) owner.TextEditor.Font = new Font((string)args.NewValue, owner.FontSize);
    }
    ...
    {
        if (obj is KaxamlTextEditor owner) owner.TextEditor.Font = new Font(owner.FontFamilyName, (float)args.NewValue);
    }
    ...
    private static void TextPropertyChanged(DependencyObject obj, DependencyPropertyChangedEventArgs args)
    {
        if (obj is KaxamlTextEditor owner)
        {
            ...
            if (!owner._resetTextInternal) owner.RaiseTextChangedEvent(newValue);
        }
    }
    ...
    {
        if (obj is KaxamlTextEditor owner) owner.TextEditor.ConvertTabsToSpaces = (bool)args.NewValue;
    }
    ...
    private static void EnableXmlFoldingChanged(DependencyObject obj, DependencyPropertyChangedEventArgs args)
    {
        if (obj is KaxamlTextEditor owner)
        {
            if ((bool)args.NewValue)
            {
                ...
                if (owner.TextEditor.Document.FoldingManager.FoldingStrategy == null) owner.TextEditor.Document.FoldingManager.FoldingStrategy = new XmlFoldingStrategy();
    ...
    {
        if (Equals(sender, this)) ShowCompletionWindow(char.MaxValue);
    }
    ...
    public void RemoveString(int beginIndex, int count)
    {
        ...
        if (CaretIndex > beginIndex) TextEditor.ActiveTextAreaControl.TextArea.Caret.Column -= count;
    ...
    {
        if (!Settings.Default.EnableCodeCompletion) return;
    ...
    private void w_ResultProvided(object? _, ResultProvidedEventArgs e)
    {
        ...
        if (!e.Canceled)
        {
            ...
            if (e.Item is XmlCompletionData xml) indentedText = xml.Text;
    ...
    public void Find(string s)
    {
        ...
        if (string.Equals(SelectedText, _findText, StringComparison.InvariantCultureIgnoreCase)) _findIndex = CaretIndex + 1;
    ...
    public void FindNext(bool allowStartFromTop)
    {
        ...
        {
            MessageBox.Show("The text \"" + _findText + "\" could not be found.");
            _findIndex = 0;
        }
    }
